...
use app\model\ChuchaiModel;
class Chuchai extends Base
{

    public function __construct(WorkModel $work,ChuchaiModel $chuchai) {
        parent::__construct();
        $this->workModel=$work;
        $this->chuchaiModel=$chuchai;

    }

    public function index()
    {
        $lists=$this->chuchaiModel->getAll();
    ...
    public function import(){
        if($_POST){
           //获取表单上传的文件
            ...
            if($info){
                $res= $this->chuchaiModel->import($info);
                if($res){
